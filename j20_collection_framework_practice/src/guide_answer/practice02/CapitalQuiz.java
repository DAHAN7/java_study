...
public class CapitalQuiz {
	...
	private void input() {
		...
		a : while(true) { // 사용자가 국가 이름에 q를 입력할때 까지 반복
			...
			// 중복 국가 인지 확인
			for(Nation n : store) {
				// 리스트에 등록된 Nation 객체의 국가 이름과
				// 사용자가 추가할려고 하는 국가이름이 일치 - 중복 국가 등록
	...
	private void quiz() {
		...
		// 정답을 다 맞히면 리스트는 비어있는 상태이므로 get() 으로
		// 꺼내올 항목이 존재 하지 않음.
		// if(store.size() == 0) {
		if(store.isEmpty()) {
			// 저장된 항목이 없음
		...
		if(quiz.getCapital().equals(answer)) {
			...
			System.out.printf("아닙니다!! %s의 수도는 %s입니다. %n",
								quiz.getCountry(),
								quiz.getCapital()
							);
		}
