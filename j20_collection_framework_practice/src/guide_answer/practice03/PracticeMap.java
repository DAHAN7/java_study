...
public class PracticeMap {
	...
	public static void main(String[] args) {
		...
		while(true) {
			...
			// 좌우 공백 제거한 문자열
			String city = strs[0].trim();			// 도시 이름 - key
			// Location - value
			String lng = strs[1].trim();			// 경도
			String lat = strs[2].trim();			// 위도
			
			// 공백 제거된 경도 위도로 Location 객체 생성
			Location location = new Location(
					Double.parseDouble(lng),		// double type으로 변경
